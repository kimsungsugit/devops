    ...
    <div style={{ display: "grid", gap: 14 }}>
      <div className="panel">
        <div className="row" style={{ justifyContent: "space-between", alignItems: "center" }}>
          <h3 style={{ margin: 0 }}>SITS Generator</h3>
        ...
        <div style={{ display: "grid", gap: 10, marginTop: 10 }}>
          {showAdvancedOverrides ? (
            <div className="card" style={{ display: "grid", gap: 10 }}>
              ...
              <div>
                <label>STP Path <span className="hint" style={{ fontSize: "0.8em" }}>(선택, 테스트 전략)</span></label>
                <div className="row" style={{ gap: 6 }}>
                  <input value={stpPath} onChange={(e) => typeof onStpPathChange === "function" && onStpPathChange(e.target.value)} placeholder="STP path (optional, test strategy)" />
              ...
              <div>
                ...
                <div className="row" style={{ gap: 6 }}>
                  ...
                  <button type="button" className="btn-outline" onClick={() => handlePickFile(onTemplatePathChange, "Select SITS template")}>Browse</button>
                </div>
              </div>
            </div>
          ) : (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 8 }}>
            ...
            <input type="number" min={1} max={20} value={maxSubcases} onChange={(e) => onMaxSubcasesChange(parseInt(e.target.value, 10) || 7)} placeholder="Max Sub-cases" />
          </div>
          {notice ? <div className="hint">{notice}</div> : null}
          {loading ? <div className="hint">{progressMsg || "Running..."} {progressPct ? `(${progressPct}%)` : ""}</div> : null}
          <div className="row">
            <button type="button" onClick={onGenerate} disabled={loading}>Generate SITS</button>
          </div>
        </div>
      </div>
