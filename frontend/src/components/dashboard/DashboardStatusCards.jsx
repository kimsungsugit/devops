const DashboardStatusCards = ({
  ...
  <div className="cards">
    <div
      className={`card clickable status-${statusTone}`}
    ...
    <div
      className={`card clickable status-${staticTone}`}
      onClick={() => handleCardClick("static")}
    >
      <div className="card-title">정적 분석 이슈</div>
      <div className="card-value">{stat.total}</div>
      <div className="card-sub">
        cpp:{stat.cpp} tidy:{stat.tidy} sem:{stat.sem}
      </div>
    </div>
    <div
      className={`card clickable status-${preflightTone}`}
      onClick={() => handleCardClick("preflight")}
    >
    ...
    <div
      className={`card clickable status-${impactTone}`}
      onClick={() => handleCardClick("change-impact")}
    >
      <div className="card-title">Change Impact</div>
      <div className="card-value">{impact.total || 0} files</div>
      <div className="card-sub">
        tests:{impact.has_tests ? "Y" : "N"} config:
        {impact.has_configs ? "Y" : "N"} build:
        {impact.has_build_files ? "Y" : "N"}
      </div>
    </div>
    <div
      className={`card clickable status-${healthTone}`}
      onClick={() => handleCardClick("report-health")}
    >
      <div className="card-title">Report Health</div>
      <div className="card-value">{health.status || "-"}</div>
      <div className="card-sub">
        missing {(health.missing || []).length} warn{" "}
        {(health.warnings || []).length}
      </div>
    </div>
    <div
      className={`card clickable status-${buildTone}`}
      onClick={() => handleCardClick("build")}
    >
      <div className="card-title">Build</div>
      <div className="card-value">
        {summary?.build?.ok ? "OK" : summary?.build?.reason || "-"}
      </div>
      <div className="card-sub">
        syntax {summary?.syntax?.ok ? "OK" : "FAIL"}
      </div>
    </div>
    <div
      className={`card clickable status-${testsTone}`}
    ...
    >
      ...
      <div className="card-sub">
        coverage{" "}
        {coverage.line_rate_pct
          ? `${coverage.line_rate_pct.toFixed(1)}%`
          : "-"}
      </div>
    </div>
    <div
    ...
    >
      ...
      <div className="card-value">
        {agent.stop_reason || (agent.enabled ? "ON" : "OFF")}
      </div>
      <div className="card-sub">
        runs {(summary?.agent_runs || []).length}
      </div>
    </div>
    <div
      className="card clickable"
      onClick={() => handleCardClick("scm")}
    >
