...
class JenkinsClient(_BaseJenkinsClient):
    ...
    def download_console_log(
        ...
        *,
        ...
        """Download Jenkins console log to dst_path."""
        selector = str(build_selector).strip()
        url = _join_url(self.job_url, f"{selector}/consoleText")
        req = self._auth_req(url, accept="text/plain")
        raw = self._open_bytes(req)
        if max_bytes and isinstance(max_bytes, int) and max_bytes > 0 and len(raw) > max_bytes:
            raw = raw[-max_bytes:]
